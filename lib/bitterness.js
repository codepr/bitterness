...
function mgAlphaAcids(hg, aa, bs) {
  return ((hg * aa * 10) / (bs));
}

function boilingFactor(og) {
  return (1.65 * (Math.exp(((og * 0.001) - 1.0) / 0.111)));
}

function boilingTimeFactor(tm) {
...
bitterness.prototype.tinseth = function() {
  ...
  mg_alpha = mgAlphaAcids(hg, aa, bs);
  b_factor = boilingFactor(og);
  bt_factor = boilingTimeFactor(tm);
