...
var bitterness = module.exports = function bitterness(hop_grams, time, alpha_acids, batch_size, original_gravity) {
  ...
  if(original_gravity > 1050) {
    this._adjustment = (original_gravity - 1050) / 2;
  }else {
    this._adjustment = 0;
  }
  this._adjustment = 0;
...
bitterness.prototype.mgAlphaAcids = function() {
  return ((this._hop_grams * this._alpha_acids * 10) / (this._batch_size));
};
/**
 *
...
bitterness.prototype.boilingFactor = function() {
  var result = 0;
  result = 1.65 * (Math.exp(((this._original_gravity * 0.001) - 1.0) / 0.111));
  return result;
};
/**
 *
...
bitterness.prototype.boilingTimeFactor = function() {
  var result = 0;
  result = (1 - Math.exp(-0.04 * this._time)) / 4.15;
  return result;
};
